...
shard_samples = [jnp.load(experiment_dir + f'data/shard_samples_seed{i}.npy') for i in shard_seeds]
samples_exact = jnp.load(experiment_dir + 'data/samples_exact.npy')
dim = samples_exact.shape[1]; n_samp = shard_samples[0].shape[1]; M = shard_samples[0].shape[0]

def print_results(method, samps):
    results = []
    for i in range(len(shard_seeds)):
...
shard_samples_swiss = [jnp.load(experiment_dir + f'data/shard_samples_swiss_seed{i}.npy') for i in shard_seeds]
...
semikde_samps = [jnp.load(experiment_dir + f'data/samples_seed{seed}_semikde.npy') for seed in shard_seeds]
...
gp_samps = [jnp.load(experiment_dir + f'data/samples_seed{seed}_gp.npy') for seed in shard_seeds]
...
diff_samps = [jnp.load(experiment_dir + f'data/samples_seed{seed}_diff.npy') for seed in shard_seeds]
